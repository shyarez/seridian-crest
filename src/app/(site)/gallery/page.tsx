...
export default function GalleryPage() {
  ...
  useEffect(() => {
    fetch('/api/gallery')
      .then(r => r.ok ? r.json() : [])
      .then((data: Array<{ _id: string; src: string; alt: string; caption: string; category: string }>) => {
        setDbImages(data.map(d => ({ id: d._id, src: d.src, alt: d.alt, caption: d.caption, category: d.category })));
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <>
      ...
      <section className="hero-gradient section-padding pb-24 text-white relative overflow-hidden">
        ...
        <div className="container-site relative z-10 text-center max-w-3xl mx-auto">
          <motion.p
            ...
            className="text-brand-highlight text-sm font-bold tracking-widest uppercase mb-4"
          >
          ...
          <motion.p
            ...
            className="text-brand-highlight/80 text-lg leading-relaxed"
          >
            A visual journey through our port operations, freight logistics, and global trade network.
          </motion.p>
        </div>
      </section>
      ...
      {/* ── Gallery Grid ──────────────────────────────────────────────────────── */}
      <section className="section-padding bg-brand-bg" aria-label="Photo gallery">
              ...
              {filtered.map((item, index) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <p className="text-white font-bold text-sm leading-snug">{item.caption}</p>
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
      ...
      <AnimatePresence>
        {activeLightboxItem && (
          ...
          >
            ...
            >
              {/* Image wrapper */}
              <div className="relative w-full rounded-2xl overflow-hidden bg-brand-alt-bg" style={{ aspectRatio: '16/10' }}>
                <Image
              ...
              <div className="mt-4 flex items-center justify-between px-1">
                <div>
                  <p className="text-white font-bold text-lg">{activeLightboxItem.caption}</p>
                  <p className="text-brand-highlight text-sm">{activeLightboxItem.category}</p>
                </div>
